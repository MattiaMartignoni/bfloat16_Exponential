...
with open('C:/Coding_Projects/ES_project/REPOSITORY/Error_analysis/Cordic/17_bit/data_file.csv','r') as csvfile:
...
with open('C:/Coding_Projects/ES_project/REPOSITORY/Error_analysis/Cordic/17_bit/err_perc.csv','r') as csvfile:
    plots = csv.reader(csvfile, delimiter=',')
    for row in plots:
        #y.append(float(row[0]))
...
print ('HIGH ERROR (for 16 bit approximation after 1000th iteration):')
...
plt.xlabel('x (iteration)')
plt.ylabel('y (absolute value of percentage error)')
#plt.title('percentage error vs. iteration (20 bits)')
plt.legend()
plt.show()
