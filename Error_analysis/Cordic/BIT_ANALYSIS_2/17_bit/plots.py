...
with open('C:/Users/matti/Desktop/TEST/BIT_ANALYSIS/17_bit/data_file.csv','r') as csvfile:
...
with open('C:/Users/matti/Desktop/TEST/BIT_ANALYSIS/17_bit/err_perc.csv','r') as csvfile:
    plots = csv.reader(csvfile, delimiter=',')
    for row in plots:
        #y.append(float(row[0]))
...
print ('total mean error 17 bits = ', errore_medio_tot)
...
plt.plot(y, label='percentage error vs. iteration (17 bits)')
plt.grid(True)
...
plt.xlabel('x (input value)')
plt.ylabel('y (absolute value of percentage error)')
#plt.title('percentage error vs. iteration (17 bits)')
plt.legend()
plt.show()
