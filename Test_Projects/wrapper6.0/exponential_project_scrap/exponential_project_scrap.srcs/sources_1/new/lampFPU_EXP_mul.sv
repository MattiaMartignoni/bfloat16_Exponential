...
input			[(1+LAMP_FLOAT_COR_F_DW)-1:0]		extShF_op1_i;
input			[(LAMP_FLOAT_COR_E_DW+1)-1:0]		extE_op1_i;
input			[$clog2(1+LAMP_FLOAT_COR_F_DW)-1:0]	nlz_op1_i;
input											isZ_op1_i;
input											isInf_op1_i;
input											isSNAN_op1_i;
input											isQNAN_op1_i;
input			[LAMP_FLOAT_COR_S_DW-1:0]			s_op2_i;
input			[(1+LAMP_FLOAT_COR_F_DW)-1:0]		extShF_op2_i;
input			[(LAMP_FLOAT_COR_E_DW+1)-1:0]		extE_op2_i;
input			[$clog2(1+LAMP_FLOAT_COR_F_DW)-1:0]	nlz_op2_i;
input											isZ_op2_i;
input											isInf_op2_i;
input											isSNAN_op2_i;
input											isQNAN_op2_i;
...
//////////////////////////////////////////////////////////////////
	...
	logic	[1+1+LAMP_FLOAT_COR_E_DW-1:0]		e_initial_res_mul_temp;		// 1+ care for ovf or 2'compl, result of the sum of exponents
	logic	[1+1+LAMP_FLOAT_COR_E_DW-1:0]		e_initial_extra_neg_temp;	// 1+ care for ovf or 2'compl, result of the sum of exponents
	logic	[1+1+LAMP_FLOAT_COR_E_DW-1:0]		e_initial_res_mul;			// 1+ care for ovf or 2'compl, result of the sum of exponents
	logic	[2*(1+LAMP_FLOAT_COR_F_DW)-1:0]		f_initial_dsp_res_mul;		// f = f_op1 * f_op2 (twice the width)
	logic	[(1+1+LAMP_FLOAT_COR_F_DW+3)-1:0]	f_initial_res_mul;			// f = f_op1 * f_op2

	// post normalization wires/regs
	logic									s_res_postNorm;
	logic	[1+1+LAMP_FLOAT_COR_F_DW+3-1:0]		f_res_postNorm;				// still keep hidden bit and overflow bit (MSB) in the bitvector
...
//////////////////////////////////////////////////////////////////
	...
	//
		...
		if (e_initial_res_mul_temp[(1+1+LAMP_FLOAT_COR_E_DW)-1])
		begin
			if(e_initial_extra_neg_temp > 1+LAMP_FLOAT_COR_F_DW+3)
			begin
		...
		f_initial_res_mul		= f_initial_dsp_res_mul[(2*(1+LAMP_FLOAT_COR_F_DW)-1) -:(1+1+LAMP_FLOAT_COR_F_DW+3/*G,R,S*/)];
		...
		stickyBit				= |f_initial_dsp_res_mul[0+: (2*(1+LAMP_FLOAT_COR_F_DW)-1) -(1+1+LAMP_FLOAT_COR_F_DW+3) ];

		f_initial_res_mul[0]	= f_initial_res_mul[0] | stickyBit;

		if (f_initial_res_mul[1/*extra bit*/+(1/*hidden 1*/+LAMP_FLOAT_COR_F_DW+3/*LSB rnd bits*/)-1] == 1)
			...
			begin // DO POST-NORM (single right shift) since e+1 < 0xff
				...
				f_res_postNorm		= {1'b0,f_initial_res_mul [1+1+LAMP_FLOAT_COR_F_DW+3-1:1]}; // shift f right by 1. NOTE: trash the sticky bit by 1 rsh of f
			end
		...
		begin
			...
			f_res_postNorm			= {f_initial_res_mul [1+1+LAMP_FLOAT_COR_F_DW+3-1:2], f_initial_res_mul[1] | f_initial_res_mul[0], 1'b0 /*old useless sticky*/ };
			e_res_postNorm			= e_initial_res_mul;
		end
		else if (f_initial_res_mul[1/*extra bit*/+(1/*hidden 1*/+LAMP_FLOAT_COR_F_DW+3/*LSB rnd bits*/)-2] == 1)
		begin
			e_res_postNorm			= '1;
	...
	//
		...
		unique if (isZeroRes)
			{s_res, e_res, f_res}	=	{isCheckSignRes, ZERO_COR_E_F, 5'b0};
