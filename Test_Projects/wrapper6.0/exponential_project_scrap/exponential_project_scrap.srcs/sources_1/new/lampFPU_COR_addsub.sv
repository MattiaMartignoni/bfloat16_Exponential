...
input										doAddSub_i;
input										isOpSub_i;
input			[LAMP_FLOAT_COR_S_DW-1:0]		s_op1_i;
input			[(LAMP_FLOAT_COR_F_DW+1)-1:0]	extF_op1_i;
input			[(LAMP_FLOAT_COR_E_DW+1)-1:0]	extE_op1_i;
input										isInf_op1_i;
input										isSNAN_op1_i;
input										isQNAN_op1_i;
input			[LAMP_FLOAT_COR_S_DW-1:0]		s_op2_i;
input			[(LAMP_FLOAT_COR_F_DW+1)-1:0]	extF_op2_i;
input			[(LAMP_FLOAT_COR_E_DW+1)-1:0]	extE_op2_i;
input										isInf_op2_i;
input										isSNAN_op2_i;
input										isQNAN_op2_i;
input										op1_GT_op2_i;
input			[(LAMP_FLOAT_COR_E_DW+1)-1:0]	e_diff_i;
//	outputs
output	logic								s_res_o;
output	logic	[LAMP_FLOAT_COR_E_DW-1:0]		e_res_o;
output	logic	[LAMP_FLOAT_COR_F_DW+5-1:0]		f_res_o;
...
//////////////////////////////////////////////////////////////////
	...
	logic	[(LAMP_FLOAT_COR_F_DW+1)+3-1:0]			f_op_rhs;					// extended F, three added LSB bits are Guard,Round,Sticky
	logic	[(LAMP_FLOAT_COR_F_DW+1)+3-1:0]			f_op_rhs_shifted;			// extended F, three added LSB bits are Guard,Round,Sticky
	logic	[1+(LAMP_FLOAT_COR_F_DW+1)+3-1:0]		f_op_rhs_shifted_2comp;		// 1 bit for 2comp, plus extended F, three LSB bits are Guard,Round,Sticky
	logic	[1+(LAMP_FLOAT_COR_F_DW+1)+3-1:0]		f_op_noShift;				// extended F, plus three added LSB bits are Guard,Round,Sticky

	// de-normalization
	logic										s_initial_res_addsub;		// the added MSB is because the add result can be 1/B < M < 2
	logic	[1+1+LAMP_FLOAT_COR_F_DW+3-1:0]			f_initial_res_addsub_temp;	// extended F + 3 LSB bits, plus the added MSB is because the add result can be 1/B < M < 2
	logic	[1+1+LAMP_FLOAT_COR_F_DW+3-1:0]			f_initial_res_addsub;		// extended F + 3 LSB bits, plus the added MSB is because the add result can be 1/B < M < 2
	logic	[LAMP_FLOAT_COR_E_DW+1-1:0]				e_initial_res_addsub;		// the added MSB is because the add result can be 1/B < M < 2
	logic	[$clog2(1+LAMP_FLOAT_COR_F_DW+3)-1:0]	leftShiftAmount;

	// post normalization wires/regs
	logic										s_res_postNorm;
	logic	[1+1+LAMP_FLOAT_COR_F_DW+3-1:0]			f_res_postNorm;			//still keep the hidden bit and the overflow bit (MSB) in the bitvector
	logic	[LAMP_FLOAT_COR_E_DW+1-1:0]				e_res_postNorm;
	logic										isOverflow_postNorm;
	logic										isUnderflow_postNorm;

	//	output next values
	logic										s_res;
	logic	[LAMP_FLOAT_COR_E_DW-1:0]				e_res;
	logic	[LAMP_FLOAT_COR_F_DW+5-1:0]				f_res;
	logic										valid;
	logic										isOverflow;
	logic										isUnderflow;
	logic										isToRound;
	...
	logic										doAddSub_r;
	logic	[LAMP_FLOAT_COR_S_DW-1:0]				s_op1_r;
	logic	[LAMP_FLOAT_COR_S_DW-1:0]				s_op2_r;
	logic										isOpSub_r;
	logic										isInf_op1_r;
	logic										isSNAN_op1_r;
	logic										isQNAN_op1_r;
	...
	logic	[1+1+LAMP_FLOAT_COR_F_DW+3-1:0]			f_initial_res_addsub_r;
	logic	[LAMP_FLOAT_COR_E_DW+1-1:0]				e_initial_res_addsub_r;
	logic	[$clog2(1+LAMP_FLOAT_COR_F_DW+3)-1:0]	leftShiftAmount_r;
...
//////////////////////////////////////////////////////////////////
	...
	begin
		...
		stickyBit				= FUNC_COR_addsub_calcStickyBit(f_op_rhs, e_diff_i); // evaluate sticky on smaller operand before Rshift
		f_op_rhs_shifted		= {f_op_rhs_shifted[1+LAMP_FLOAT_COR_F_DW+3-1: 1], f_op_rhs_shifted[0]|stickyBit}; // sticky applied after right shift
	...
	// perform REGULAR f add/sub. if sub or sign of the two operands differs do perform 2's complement of one operand!
		...
		f_op_rhs_shifted_2comp	= doOpSub ? ({1'b0,f_op_rhs_shifted} ^ {(1/*for 2comp*/+(1+LAMP_FLOAT_COR_F_DW/*extended*/)+3){1'b1}}) + 1'b1 : {1'b0,f_op_rhs_shifted};
		...
		f_initial_res_addsub	= doOpSub ? {1'b0,f_initial_res_addsub_temp[((1/*hidden*/+LAMP_FLOAT_COR_F_DW)+3/*LSB*/)-1:0]} : f_initial_res_addsub_temp;
	...
	//
		...
		leftShiftAmount			= FUNC_COR_AddSubPostNorm_numLeadingZeros(f_initial_res_addsub);
...
//	<-	REGISTERED HERE	->
		...
		if (f_initial_res_addsub_r[1/*extra bit*/+(1/*hidden 1*/+LAMP_FLOAT_COR_F_DW+3/*LSB rnd bits*/)-1] == 1)
			...
			begin // DO POST-NORM (single right shift) since e+1 < 0xff
				...
				f_res_postNorm	= {1'b0,f_initial_res_addsub_r [1+1+LAMP_FLOAT_COR_F_DW+3-1:1]}; // shift f right by 1. NOTE: trash the sticky bit by 1 rsh of f
			end
		else if (f_initial_res_addsub_r[(1+1+LAMP_FLOAT_COR_F_DW+3/*LSB rnd bits*/)-2] == 1) //no right shift, no left shift -> no postnorm, thus fix the sticky bit for rounding
		begin
			//check if e=0 to signal zero or underflow
			f_res_postNorm	= {f_initial_res_addsub_r [1+1+LAMP_FLOAT_COR_F_DW+3-1:2], f_initial_res_addsub_r[1] | f_initial_res_addsub_r[0], 1'b0 /*old useless sticky*/ };
	...
	// compute if nan or infinite res
		{isCheckNanInfValid, isCheckInfRes, isCheckNanRes, isCheckSignRes} = FUNC_COR_calcInfNanResAddSub(
					isOpSub_r,												/*operator*/
		...
		unique if (isCheckInfRes)
			{s_res, e_res, f_res}	=	{isCheckSignRes, INF_COR_E_F, 5'b0};
		else if (isCheckNanRes)
			{s_res, e_res, f_res}	=	{isCheckSignRes, QNAN_COR_E_F, 5'b0};
		else
			{s_res, e_res, f_res}	=	{s_res_postNorm, e_res_postNorm[LAMP_FLOAT_COR_E_DW-1:0], f_res_postNorm};
		valid		= doAddSub_r;
